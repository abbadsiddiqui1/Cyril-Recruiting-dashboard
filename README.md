...
- **Merged in from [sndsh404/summer-2027-internships](https://github.com/sndsh404/summer-2027-internships):** a community-maintained list of real, currently-open Summer 2027 postings with direct apply links and posted dates. Added 127 company/role entries total (67 brand-new companies, plus 20 additional specific-role entries for companies already in the base list, e.g. Palantir's 6 open reqs alongside its original generic entry), which introduced a new **Quant/Trading** tier since a large share of that list is quant trading firms (Optiver, Two Sigma, IMC Trading, Susquehanna, etc.).
- **Dedup approach:** entries were matched by company name against the existing 97. Brand-new companies were merged in directly. Companies that already existed in the base list but had different specific role(s) in the second source were reviewed manually (some updated/merged, most added as additional rows alongside the original placeholder entry) — some near-duplicate rows for the same company may still need manual cleanup/merging in the UI over time.
